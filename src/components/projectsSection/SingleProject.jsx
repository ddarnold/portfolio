...
const SingleProject = ({ name, year, align, image, link, description}) => {
  return (
    ...
    >
      <div>
        <h2 className="md:text-3xl sm:text-2xl text-orange sm:text-center">{name}</h2>
        <h2
        ...
        <p
          className={`text-white mt-2 sm:text-center ${
            align === "left" ? "md:text-right" : "md:text-left"
          }`}
        >
          {description}
      ...
      <div className="max-h-[220px] max-w-[400px] rounded-xl overflow-hidden hover:scale-110 transform transition-all duration-500 relative border border-white">
        <div className="w-full h-full bg-cyan opacity-40 absolute top-0 left-0 hover:opacity-0 transition-all duration-500 md:block sm:hidden"></div>
